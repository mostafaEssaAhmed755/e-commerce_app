                ...
                <div class="table-responsive">
                    <table class="table table-sm">
                        ...
                        <tr v-for="pa in productAttributes">
                            <td style="width: 25%" class="text-center">{{ pa.attributeName}}</td>
                            <td style="width: 25%" class="text-center">{{ pa.attributeValueName}}</td>
...
export default {
    ...
    methods: {
        loadProductAttributes(id) {
            let _this = this;
            axios.post(`/admin/products/${id}/attributes`).then (function(response){
        ...
        loadAttributes() {
            let _this = this;
            axios.get('/admin/products/attributes').then (function(response){
        ...
        selectAttribute(attribute) {
            ...
            axios.post(`/admin/products/attributes/${attribute.id}/values`).then (function(response){
        ...
        addProductAttribute() {
            if (this.currentQty === null || this.currentPrice === null) {
                ...
                let data = {
                    ...
                    attribute_value_id:  this.currentValue,
                    quantity: this.currentQty,
                    price: this.currentPrice,
                };

                axios.post(`/admin/products/${this.productid}/attributes/add`, data).then (function(response){
                    if (response.data.status == 'success') {
                        ...
                        _this.$swal("Oops! " + response.data.message, {
                            icon: "error",
                        });
                    }
        ...
        deleteProductAttribute(pa) {
            ...
            this.$swal({
                ...
                if (willDelete) {
                    axios.post(`/admin/products/${pa.id}/attributes/delete`).then (function(response){
                        if (response.data.status === 'success') {
