                        ...
                        <div class="row no-gutters">
                            ...
                            <aside class="col-sm-7">
                                <article class="p-5">
                                    ...
                                    <hr>
                                    <form action="{{ route('cart.add.item', $product->id) }}" method="POST" role="form" id="addToCart">
                                        @csrf
                                        <div class="row">
                                            <div class="col-sm-12">
                                                <dl class="dlist-inline">
                                                    @foreach($attributes as $attribute)
                                                        @php $attributeCheck = in_array($attribute->id, array_unique($product->attributes->pluck('attribute_id')->toArray())) @endphp
                                                        @if ($attributeCheck)
                                                            ...
                                                            <dd>
                                                                <select class="form-control form-control-sm option" style="width:180px;" name="{{ strtolower($attribute->name ) }}">
                                                                    ...
                                                                    @foreach($product->attributes as $productAttr)
                                                                        @if ($productAttr->attribute_id == $attribute->id)
                                                                            <option
                                                                                data-price="{{ $productAttr->price }}"
                                                                                value="{{ $productAttr->attributesValue->value }}"> {{ ucwords($productAttr->attributesValue->value . ' +'. $productAttr->price) }}
                                                                            </option>
                                                                        @endif
                                                                    @endforeach
                                                                </select>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.option').change(function () {
                ...
                let extraPrice = 0;
                $('.option').each(function () {extraPrice += parseFloat($(this).find(':selected').data('price'));});
